#include <string.h>
#include <stdlib.h>
#include "format-double-vector.h"
...
static inline void
buf_reserve (Buf *buf, int n)
{
  if (buf->len + n > buf->alloc)
    buf->str = realloc (buf->str, buf->alloc <<= 1);
}

static inline void
buf_append_c (Buf *buf, char c)
{
  buf_reserve (buf, 1);
  buf->str[buf->len++] = c;
}
...
static const char *exponents[2048] = {
  "-1022",	/* denormals */
  "-1022",
  "-1021",
  "-1020",
  ...
  "+1022",
  "+1023",
  "+1024"	/* not actually used (inf/nan) */
};

static inline void
buf_append_hex_double (Buf *buf, const double *x)
{
  const union DoubleIEEE754 *d = (const union DoubleIEEE754 *) x;
...
char *
format_double_vector (const char *fmt, const double *v)
{
  Buf buf;
  buf_init (&buf, 4096);
  for ( ; *fmt; ++fmt)
    {
      if (*fmt == '@')
	buf_append_hex_double (&buf, v++);
      else
	buf_append_c (&buf, *fmt);
    }
  buf_append_c (&buf, '\0');
  return buf.str;
}
